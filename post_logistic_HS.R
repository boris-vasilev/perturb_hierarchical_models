...
fit <- readRDS(here(glue("/rds/project/rds-csoP2nj6Y6Y/biv22/models/{cells}/{ash}logit_{model}{eff}_HS.rds")))

png(here(glue("plots/{cells}/traceplot_logit_{model}_HS.png")), width= 1000, height = 1000)
plot(fit)
dev.off()
...
png(here(glue("plots/{cells}/logit_parameter_shrinkage_{model}_HS.png")), width = 1080, height = 1080)
...
png(here(glue("plots/{cells}/logit_{model}_intercepts_HS.png")), width = 3080, height = 1080)
...
png(here(glue("plots/{cells}/logit_{model}_intercepts_significant_HS.png")), width = 580, height = 580)
...
png(here(glue("plots/{cells}/logit_{model}_slopes_HS.png")), width = 3080, height = 1080)
p + theme_void() + theme(text = element_text(size = 10), axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()
...
png(here(glue("plots/{cells}/logit_{model}_slopes_significant_HS.png")), width = 580, height = 580)
p + theme_bw() + theme(text = element_text(size = 10), axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()
