...
gene_interactions <- read_csv(here("data/perturb/pairs/Jurkat/pertubration_pairs.csv"))

# Read eQTL pairs (cis-eQTL significant, transs-eQTL any)
...
gene_interactions_pathways <- gene_interactions_filtered %>%
  dplyr::select(effect, perturbation, intersecting_pathways) %>%
