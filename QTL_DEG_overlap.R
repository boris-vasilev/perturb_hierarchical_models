...
write_csv(perturbation_effect_df, here("data/perturb/pertubration_pairs.csv"))
...
cis_eQTL.perturbations.all <- cis_eQTL.perturbations.all %>% dplyr::select(SNP, Pvalue, GeneSymbol)
...
QTL.pairs <- merged.QTL %>%
  unnest_longer(c(Pvalue.perturb, GeneSymbol.perturb)) %>%
  unnest_longer(c(Beta, Pvalue.effect, GeneSymbol.effect)) %>%
...
pvals.false %>% sample(1000000) %>% qq(., main="Trans-eQTL of false perturb-seq affected genes")
dev.off()
