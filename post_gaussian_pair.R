...
parser$add_argument("--perturb", type = "character", help = "Perturbation to plot", required = TRUE)
...
dat <- read_csv(here(glue("data/perturb/pairs/{cells}/gaussian_{response}_dat.csv")))
dat <- dat %>% filter(perturb == perturb) %>% mutate(pair = paste(perturb, effect))

fit <- readRDS(here(glue("models/{cells}/gaussian_{response}_{model}.rds")))



png(here(glue("plots/{cells}/traceplot_gaussian_{response}_{model}.png")), width= 1000, height = 500)
plot(fit)
dev.off()

#png(here(glue("plots/{cells}/pp_check_gaussian_{response}_{model}.png")), width= 1000, height = 500)
#p <- pp_check(fit)
#p
#dev.off()

partially_pooled_params <-
  # with this line we select each of the pairation posterior mean (i.e., Estimate)
  # for both `Intercept` and `x - pair log2FC`
  coef(fit)$pair[ , 1, 1:2] %>%
...
p1 <-
  ...
  geom_point(aes(group = pair, color = pooled), alpha = 0.2, size = 3) +
  geom_line(aes(group = pair), linewidth = 1/4, alpha = 0.2) +
  scale_color_manual("Pooled?", values = c("blue", "red")) +
  coord_cartesian(xlim = range(params$Intercept),
                  ylim = range(params$Slope))

png(here(glue("plots/{cells}/gaussian_parameter_shrinkage_{response}_{model}.png")), width = 1080, height = 1080)
p1 + theme_bw() + theme(text = element_text(size = 20))
dev.off()


# Extract intercepts and slopes with their 95% credible intervals
partially_pooled_intercepts.confidence <- coef(fit)$pair[,,1] %>%
	data.frame %>%
	select(Estimate, Q2.5, Q97.5) %>%
	rownames_to_column("pair")

partially_pooled_slopes.confidence <- coef(fit)$pair[,,2] %>%
	data.frame %>%
	select(Estimate, Q2.5, Q97.5) %>%
	rownames_to_column("pair")

p <- ggplot(partially_pooled_intercepts.confidence, aes(x = pair, y = Estimate)) +
...
png(here(glue("plots/{cells}/gaussian_{response}_{model}_intercepts.png")), width = 3080, height = 1080)
...
p <- ggplot(partially_pooled_intercepts.significant, aes(x = pair, y = Estimate)) +
...
png(here(glue("plots/{cells}/gaussian_{response}_{model}_intercepts_significant.png")), width = 580, height = 580)
p + theme_bw() + theme(text = element_text(size = 10), axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()

p <- ggplot(partially_pooled_slopes.confidence, aes(x = pair, y = Estimate)) +
  geom_point(size = 3) +  # plot the estimates as points
  geom_errorbar(aes(ymin = Q2.5, ymax = Q97.5), width = 0.2) +  # plot credible intervals
...
png(here(glue("plots/{cells}/gaussian_{response}_{model}_slopes.png")), width = 3080, height = 1080)
p + theme_void() + theme(text = element_text(size = 10), axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()
...
p <- ggplot(partially_pooled_slopes.significant, aes(x = pair, y = Estimate)) +
  geom_point(size = 2) +  # plot the estimates as points
  geom_errorbar(aes(ymin = Q2.5, ymax = Q97.5), width = 0.2) +  # plot credible intervals
...
png(here(glue("plots/{cells}/gaussian_{response}_{model}_slopes_significant.png")), width = 580, height = 580)
p + theme_bw() + theme(text = element_text(size = 10), axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()
