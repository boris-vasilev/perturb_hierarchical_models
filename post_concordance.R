...
supported_models <- c("vivs_student_concordance",
		      "vivs_concordance",
"vifs_concordance")
...
dat <- dat %>% mutate(c = sign(x) != sign(y))

fit <- readRDS(here(glue("models/{bulk}{cells}/sign_ratio_{model}{eff}.rds")))

png(here(glue("plots/{bulk}{cells}traceplot_{ash}{bulk}concordance_{model}.png")), width= 1000, height = 1000)
plot(fit)
dev.off()
...
un_pooled_params <- dat %>%
  group_by(perturb) %>%
  do(tidy(glm(c ~ x, data = ., family='binomial'))) %>%  # fit lm per group and tidy the output
...
png(here(glue("plots/{bulk}{cells}{ash}{bulk}concordance_parameter_shrinkage_{model}.png")), width = 1080, height = 1080)
...
png(here(glue("plots/{bulk}{cells}{ash}{bulk}concordance_{model}_intercepts.png")), width = 3080, height = 1080)
...
png(here(glue("plots/{bulk}{cells}{ash}{bulk}concordance_{model}_intercepts_significant.png")), width = 580, height = 580)
...
png(here(glue("plots/{bulk}{cells}{ash}{bulk}concordance_{model}_slopes.png")), width = 3080, height = 1080)
p + theme_void() + theme(text = element_text(size = 10), axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()
...
png(here(glue("plots/{bulk}{cells}{ash}{bulk}concordance_{model}_slopes_significant.png")), width = 580, height = 580)
p + theme_bw() + theme(text = element_text(size = 10), axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()
